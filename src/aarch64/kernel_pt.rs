...
//
// `*const u8` is a pointer type to a byte. There is no special meaning that we choose to use `u8`, just because
// `u8`'s size is exactly 1 byte, which makes it easier to calculate the byte offset.
//
